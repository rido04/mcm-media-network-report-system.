           ...
           <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
               ...
               @forelse ($mediaPlacement as $item)
                   <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                       ...
                       <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                           {{ number_format($item->dailyImpression?->sum('impression') ?? 0) }}
                       </td>
                       <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                           {{ number_format($item->dailyImpression?->avg('impression') ?? 0, 2) }}
                       </td>
                   </tr>
               @empty
                   <tr>
                       <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">No Media
                           Placement yet.
                       </td>
                   </tr>
               @endforelse
       ...
       <div class="overflow-x-auto">
           <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
               ...
               @forelse ($mediaStatistics as $item)
                   <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                       <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $item->media }}</td>
                       <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $item->start_date }}</td>
                       <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $item->end_date }}</td>
                       <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
               ...
               @empty
                   <tr>
                       <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">No Media Plans
                           yet.
                       </td>
                   </tr>
               @endforelse
           ...
           <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
               ...
               @forelse ($playLogs as $log)
                   <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                       ...
                       <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $log->media_name }}</td>
                       <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $log->play_date }}</td>
                       <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $log->longitude }}</td>
                       <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $log->latitude }}</td>
                       <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
               ...
               @empty
                   <tr>
                       <td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">No Play Log yet.
                       </td>
                   </tr>
               @endforelse
               </tbody>
           ...
           @forelse ($documentations as $doc)
               <div
                   ...
                   <div class="p-4">
                       ...
                       <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Client: {{ $doc->user->name ?? '-' }}</p>
                       <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Upload: {{ $doc->created_at->format('d M Y') }}
                       </p>
                   </div>
               </div>
           @empty
               <div class="col-span-3 py-8 text-center">
                   <p class="text-gray-500 dark:text-gray-400">There's no documentation yet.</p>
               </div>
           @endforelse
       </div>
   </div>
